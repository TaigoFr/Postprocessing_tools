...
def get_weyl4_mode(file_path):
    ...
    #     if times[i] > 300:
    #         times_changed.append(times[i])
    #         weyl4_changed.append(weyl4[i])
    # return times_changed, weyl4_changed
...
def convert_weyl4_to_amplitude_and_phase(times, weyl4, start_monoticity):
    ...
    for i, time, file in zip(range(len(weyl4)), times, weyl4):
        ...
        if len(phases) > 0 and time > start_monoticity:
            ...
            # if i > 1000:
            #     print("Iter", i)
            #     print(phase_old, phase)
            #     print(phase_minus, phase_orig, phase_plus)
            #     print(diff_minus, diff_orig, diff_plus)
...
base_folder = "/media/taigofr/SanDiskExtremeSSD/MineToo/PhD Extra/BSF/run17/"
